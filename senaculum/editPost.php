 ...
 ***************************************************************************/
	...
	{
		$error->guide("Error in url", "There is an error in the url, make sure the the url you enterd is correct if you followed a link on this forum please contact the admin.", false);
	}
	
	$post = $posts->getOne($_GET['id'], true);
	...
	$forumID = $posts->forumIDFromPostID($_GET['id']);
	$threads = $thread->getOne($post['threadID'],true);
	...
	if((!$forumVariables['adminInlogged'] && !$post['authorID'] == $forumVariables['inloggedMemberID']) || (!$auth->moderator($forumID,"edit") && !$post['authorID'] == $forumVariables['inloggedMemberID']))
	{
		$error->guide($lang['notLoggedInModeratorOrCreatorOfPost1'], $lang['notLoggedInModeratorOrCreatorOfPost2'], false);
	}
	...
	elseif(isset($_POST['headline']))
	{
		...
		elseif(isset($_POST['deleteAttachment'])) {
			...
			if(!empty($errorHeadline)||!empty($errorText)) {
				$headline = $_POST['headline'];
				$text = $_POST['text'];
				if(isset($_POST['disableBBCode']))
					$disableBBCode = true;
			...
			if(empty($errorHeadline)&&empty($errorText)) {
				...
				$nextAction = "posts.php?id=".$post['threadID']."&pid=".$_GET['id']."#".$_GET['id'];
				$error->done($lang['postEdited1'],$lang['postEdited2'],$nextAction);
			}
		}	
	}
	else {
	...
	$title = $lang['editPostHeading'];
	$heading = $lang['editPostHeading'];
	$help = $lang['editPostHelp'];
