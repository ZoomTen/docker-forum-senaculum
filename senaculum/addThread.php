 ...
 ***************************************************************************/
	...
	$addThread = $permission->permission($_GET['id'],"thread");
	if(!$forumVariables['inlogged'] && !$addThread) {
	...
	if($forumSettings['attachmentsActivated'])
		$attach = $permission->permission($_GET['id'],"attach");
	else
		$attach = false;
		
	$poll = $permission->permission($_GET['id'],"poll");
	...
	if(isset($_POST['preview'])) {
		...
		if(isset($_POST['attachSign']))		
			...
			$attachSign = false;	
		if(!empty($_POST['ownerGuestName']))
			$ownerGuestName = $_POST['ownerGuestName'];
		$type = $_POST['type'];
	...
	elseif(isset($_POST['headline'])) {
		...
		elseif(isset($_POST['deleteAttachment'])) {
			...
			if(!empty($_POST['ownerGuestName']))
				$errorOwnerGuestName=$control->text($_POST['onwerGuestName'],0,20);
			if(isset($_POST['enablePoll'])) {
			...
			if(!empty($errorHeadline)||!empty($errorText)||!empty($errorOwnerGuestName)||!empty($errorPollQuestion)||!empty($errorPollOptions)||!empty($errorPollRunFor))
			{
				$headline = $_POST['headline'];
				$text = $_POST['text'];
				if(isset($_POST['disableBBCode']))
			...
			else
			{
				...
				if($attach && isset($_POST['enableAttachments']) && !empty($_POST['attachmentNumber']) && is_array($_POST['attachmentFilename']) && !empty($_POST['attachTempname'])) {
					...
					if(isset($_POST['enablePoll']) && $permission->permission($_GET['id'],"poll")) 
						$thread->add($_POST['headline'], $_POST['text'],$_GET['id'], $type, $disableBBCode, $disableSmilies, $notifyWhenReply, $attachSign, $ownerGuestName, $attachments, $_POST['pollQuestion'], $_POST['pollOptions'], $_POST['pollRunFor']);
					else
						$thread->add($_POST['headline'], $_POST['text'],$_GET['id'], $type, $disableBBCode, $disableSmilies, $notifyWhenReply, $attachSign, $ownerGuestName, $attachments);
				}
				else {
					if(isset($_POST['enablePoll']) && $permission->permission($_GET['id'],"poll")) 
						$thread->add($_POST['headline'], $_POST['text'],$_GET['id'], $type, $disableBBCode, $disableSmilies, $notifyWhenReply, $attachSign, $ownerGuestName, "", $_POST['pollQuestion'], $_POST['pollOptions'], $_POST['pollRunFor']);
					else
						$thread->add($_POST['headline'], $_POST['text'],$_GET['id'], $type, $disableBBCode, $disableSmilies, $notifyWhenReply, $attachSign, $ownerGuestName, "");
				}		
				$nextAction = "threads.php?id=".$_GET['id'];
				$error->done($lang['threadCreated1'],$lang['threadCreated2'],$nextAction);
			}
		}
	}	
	...
	$title = $lang['createNewThread'];
	$heading = $lang['createNewThread'];
	$help = $lang['addThreadHelp'];
...
<form action="addThread.php?id=<?php echo $_GET['id']?>" method="post"<?php if($poll) echo " onsubmit=\"submitPolls();\""; ?>  enctype="multipart/form-data">
	<?php 
	if($preview) {
							...
							<table width="100%" cellspacing="0" cellpadding="3">
								<tr>
									...
									<td valign="top" class="postListAuthor1" rowspan="3">
										...
										?>
										<?php if(!empty($ownerGuestName)) echo "<b>".$ownerGuestName."</b><br/>"; ?><span class="postListGuest"><?php echo $lang['guest']; ?></span><br/>
										<?php
										}
										?>
										<br/>
				...
				<table cellspacing="0" cellpadding="3" class="guideInputArea">
					...
					<tr>
						<td class="guideInputs">
							<?php echo $lang['yourNameOptional']; ?>: <span class="errorText"><?php if(!empty($errorOwnerGuestName)) echo $errorOwnerGuestName; else echo "&nbsp;"; ?></span><br/>
							<input name="ownerGuestName" type="text" size="40" value="<?php echo $ownerGuestName; ?>" class="guideTextFields"/>
						</td>
					</tr>
					<?php
					}
